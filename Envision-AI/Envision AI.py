...
def create_map_widget(parent_frame):
    map_widget = TkinterMapView(parent_frame, width=800, height=600, corner_radius=0)
    map_widget.set_position(43.238949, 76.889709)  # Алматы
    map_widget.set_zoom(11)
    map_widget.place(x=250, y=80)
    return map_widget
...
def get_video_capture():
    return cv2.VideoCapture(DROIDCAM_URL if USE_DROIDCAM else 0)
...
from district_map import create_map_widget
...
def capture_image_and_save(name):
    ...
    # cap = cv2.VideoCapture(DROIDCAM_URL if USE_DROIDCAM else 0)
...
def run_computer_vision():
    ...
    model = YOLO('yolov5x.pt')
    model.to(device)  # перенос устройства после инициализации
...
def Camera():
    ...
    head3 = tk.Label(text="                        Camera Feed Section                            ",
                     fg="White",
                     bg="#424949", font=('Century Gothic', 17), height=1)
    head3.place(x=0, y=0)
    ...
    if not cap.isOpened():
        ...
        lmain = tk.Label(text="No camera connected.. \n Connect a camera to start computer vision",
                         font=('Century Gothic', 14), bg="#D0D3D4")
        ...
        lmain = tk.Label(highlightthickness=0, bd=0)
...
def Map_Page1():
    show_frame(MapPage)
    response = "Map page selected"
    threading.Thread(target=speak1, args=(response,)).start()
    create_map_widget(MapPage)
    t = threading.Thread(target=speak1, args=(response,))
    t.start()
...
filename = ImageTk.PhotoImage(Image.open('images\\Background11111.png'))

filename1 = ImageTk.PhotoImage(Image.open('images\\Background_CV11.png'))

filename2 = ImageTk.PhotoImage(Image.open('images\\Background_AI2.png'))

# filename3 = ImageTk.PhotoImage(Image.open('images\\background_map .png'))


########################################################--------MAIN MENU PAGE---------############################################################################

# MainPage = tk.Frame(window, width=1366, height=768)
MainPage = tk.Frame(window, width=1063, height=500)
MainPage.grid(row=0, column=0, stick='nsew')
background_label = tk.Label(MainPage, image=filename)
background_label.place(x=0, y=0, relwidth=1, relheight=1)

button1 = Button(MainPage, text="C o m p u t e r  V i s i o n", font=('Times New Roman', 14),
                 command=lambda: ComputervisionPage1(), relief=RAISED, bg="#FFBBFF", fg="black",
                 width=19, height=1, borderwidth=0)
button1.bind("<Enter>", increase_size)
button1.bind("<Leave>", decrease_size)
button1.pack()
button1.place(x=100, y=490)

button2 = Button(MainPage, text="A I  A s s i s t a n t", font=('Times New Roman', 14),
                 command=lambda: AssistantPage1(), relief=RAISED, bg="#FFBBFF", fg="black",
...
button2.pack()
button2.place(x=320, y=490)


button3 = Button(MainPage, text="M A P", font=('Times New Roman', 14),
                    command=Map_Page1, relief=RAISED, bg="#FFBBFF", fg="black",
                    width=18, height=1, borderwidth=0)
...
author = tk.Label(MainPage, text="made by Yedige Mussabayev", bg="#0A0A0A", fg="#8A2BE2")
author.place(x=30, y=50)


#######################################--------------MAP PAGE------------------#####################################

MapPage = tk.Frame(window, width=1366, height=768)
MapPage.grid(row=0, column=0, sticky='nsew')

# background_label3 = tk.Label(MapPage, image=filename3)
# # background_label3.image = filename3
...
back = tk.Button(MapPage, text="Back", command=lambda: Back_btn(), fg="white", bg="#404040",
                 width=20, height=1, activebackground="white", font=('Century Gothic', 15))
back.place(x=320, y=680)



#########333########################--------------Computer Vision PAGE------------------############################################################
...
StartCV = tk.Button(ComputerVisionPage, text="➔ Start Computer Vision", command=lambda: computervision1(), fg="white",
                    bg="#004aad",
                    width=24, height=1, activebackground="white", font=('Century Gothic', 15))
StartCV.place(x=100, y=250)

StartFace = tk.Button(ComputerVisionPage, text="➔ Face Recognition", command=lambda: [show_frame(SavedPersonsFrame),thumbnail_treeview.update_list(), load_known_faces1()], fg="white",
                    bg="#004aad",
                    width=24, height=1, activebackground="white", font=('Century Gothic', 15))
StartFace.place(x=100, y=450)
...
image = Image.open("images\\Background_AI2.png")
...
user_Name = Label(AssistantPage)
user_Name.pack()
user_Name.place(x=2, y=2)

user_label = Label(AssistantPage, text="", font=('Century Gothic', 14), fg='black', height=2, width=65, wraplength=500,
                   anchor="w", padx=20)
user_label.pack()
user_label.place(x=50, y=400)

Computer_label = Label(AssistantPage)
Computer_label.pack()
Computer_label.place(x=2, y=2)
...
response_label = Label(AssistantPage, text=" " )
response_label.pack()
response_label.place(x=2, y=2)
